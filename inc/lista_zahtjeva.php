...
. " AND zahtjevi.status not in (7,8,9)"
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
	...
	<?
	  while($red_lista_zahtjeva = mysql_fetch_object($query_lista_zahtjeva)){
		...
		$sql_blink = "SELECT DATEDIFF(DATE_ADD(CURDATE(), INTERVAL 0 DAY),datum_promjene) razlika_datum FROM istorija_zahtjeva WHERE tip_promjene = 'status' and nova_vrijednost = 6 and id_zahtjev = ".$red_lista_zahtjeva->id_zahtjev." ORDER BY datum_promjene desc";

        $query_blink = mysql_query($sql_blink) or die( "Greska sql_blink = $sql_blink ".mysql_error());
		$red_blink = mysql_fetch_object($query_blink);
		...
		if($tip_korisnika == 2){
			echo "<td width=10% class=sadrzaj><a href=\"\" class=fixedTip title=\"Tip zahtjeva: $tip_zahtjeva\">$red_lista_zahtjeva->id_zahtjev</a></td>";
		}else{
		...
		if($tip_korisnika == 2){
			echo "<td width=30% class=sadrzaj><a href = index.php?s=prikaz_zahtjeva&z=$red_lista_zahtjeva->id_zahtjev class=fixedTip title=\"Podnosilac: $red_lista_zahtjeva->txt_podnosilac_zahtjeva, Izvršilac: $izvrsilac\">$red_lista_zahtjeva->naslov_zahtjeva</a></td>";
		}else{
		...
		if ($tip_korisnika == 2 && $red_blink->razlika_datum > 15)
		  echo "<td width=20% class=sadrzaj><span style=color:#A30000>$red_lista_zahtjeva->status</span></td>";
		elseif ($tip_korisnika == 1 && ($red_lista_zahtjeva->status == 'Komentar korisnik' || $red_lista_zahtjeva->status == 'Evidentiran'))
		  echo "<td width=20% class=sadrzaj><span style=color:#A30000>$red_lista_zahtjeva->status</span></td>";
		elseif (($tip_korisnika == 3 OR $tip_korisnika == 3) && ($red_lista_zahtjeva->status == 'Komentar PING' || $red_lista_zahtjeva->status == 'Isporucen na testiranje'))
		  echo "<td width=20% class=sadrzaj><span style=color:#A30000>$red_lista_zahtjeva->status</span></td>";
		else
