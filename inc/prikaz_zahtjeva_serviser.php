...
if($_POST['posted'] == '1'){
	...
	$sql_status = "SELECT status, serviser FROM zahtjevi WHERE id_zahtjev = ".$_GET['z'].";";
	...
	if(($stari_status != $_POST['status']) OR !(empty($odg_osoba))){  
...
// pomocni niz statusa za formu
	$statusi = array();
	$i=0;
	$sql_statusi = "SELECT id_status, status FROM statusi WHERE id_status IN (3,6,".$red_zahtjev->status.") ORDER BY id_status";
...
<input type="hidden" name ="posted" value=1>
 	<fieldset>
		...
		<label2 for="login"><? echo $red_zahtjev->prioritet; ?>&nbsp;</label2>			
		<div class="clear"></div>
 	...
 	<fieldset>
		...
		<label for="password">Tip zahtjeva : </label>
			...
			<p id="rok_error" class="error">Morate unijeti krajnji rok!</p>
			...
			<label for="password">&nbsp;</label>
			 <div id="buttonpos">
			       <input type="submit" style="margin: -20px 0 0 287px;" class="button2" name="commit" id = "promijeni_serviser" value="Sačuvaj promjene"/>	
			</div>
