...
if($_POST['posted'] == '1'){
	...
	$sql_status = "SELECT status FROM zahtjevi WHERE id_zahtjev = ".$_GET['z'].";";
	...
	if ($_POST['tip_zahtjeva'] == '' || $_POST['vrsta_zahtjeva'] == ''){
	$sql_update_zahtjev = "UPDATE zahtjevi SET status = ".$_POST['status'].", "    
		...
		."broj_zahtjeva_korisnika = '".$_POST['broj_zahtjeva']."'"
...
// pomocni niz statusa za formu
	$statusi = array();
	$i=0;
	$sql_statusi = "SELECT id_status, status FROM statusi WHERE id_status IN (1,7,".$red_zahtjev->status.") ORDER BY id_status";
...
<? if ($red_zahtjev->status != 1) {?>

        <label for="login">Tip zahtjeva : </label>
		<label2 for="login"><? echo $red_zahtjev->txt_tip_zahtjeva; ?>&nbsp;</label2>			
		<div class="clear"></div>

        <label for="login">Vrsta zahtjeva : </label>
		<label2 for="login"><? echo $red_zahtjev->vrsta_zahtjeva; ?>&nbsp;</label2>
		<div class="clear"></div>
