<?
if($_POST['posted'] == '1'){
	$krajnji_rok = "'".make_my_time($_POST['krajnji_rok'])."'";	
	...
	$sql_update_zahtjev = "UPDATE zahtjevi SET tip_zahtjeva = ".$_POST['tip_zahtjeva'].", vrsta_zahtjeva = '".$_POST['vrsta_zahtjeva']."', "
	    ."status = ".$_POST['status'].", serviser = '".$_POST['izvrsioc']."', lokacija_izvrsenja = '".$_POST['lokacija']."', "
		."modul = '".$_POST['modul']."', zadnji_rok = ".$krajnji_rok.", broj_sati = ".$broj_sati.", "
		."opis_realizacije = '".addslashes($_POST['opis_realizacije'])."', user_izmijenio = '".$user_izmijenio."', "
		."nacin_realizacije = '".$_POST['nacin_realizacije']."', prioritet = '".$_POST['prioritet']."'"
...
if($_POST['posted'] == '2'){
	...
	$sql_update_zahtjev = "UPDATE zahtjevi SET status = 4, user_izmijenio = '".$user_komentarisao."' WHERE id_zahtjev = ".$_GET['z'].";";		
	$query_update_zahtjev = mysql_query($sql_update_zahtjev) or die("GRESKA: sql_update_zahtjev $sql_update_zahtjev ".mysql_error());
...
// pomocni niz nacina realizacije za formu
	$nacin_realizacije = array();
	$i=0;
	$sql_nacin_realizacije = "SELECT id_nacin_realizacije, nacin_realizacije FROM nacini_realizacije ORDER BY id_nacin_realizacije";
	$query_nacin_realizacije = mysql_query($sql_nacin_realizacije) or die( "Greska: sql_nacin_realizacije = $sql_nacin_realizacije ".mysql_error());

	while($red_nacin_realizacije = mysql_fetch_object($query_nacin_realizacije)){

		$nacin_realizacije[$i][0] = $red_nacin_realizacije->id_nacin_realizacije;
		$nacin_realizacije[$i][1] = $red_nacin_realizacije->nacin_realizacije;
		$i++;
	}	
...
// pomocni niz statusa za formu
	$statusi = array();
	$i=0;
	$sql_statusi = "SELECT id_status, status FROM statusi WHERE id_status IN (1,2,3,6,7,9,".$red_zahtjev->status.") ORDER BY id_status";
...
<input type="hidden" name ="posted" value=1>
 	<fieldset>
		...
		<label for="login">Broj korisnika : </label>
		<label2 for="login"><? echo $red_zahtjev->broj_zahtjeva_korisnika; ?>&nbsp;</label2>			
		<div class="clear"></div>		
		...
		<label for="login">Preferirani rok : </label>
		<label2 for="login"><? if (date ( "d.m.Y" , strtotime($red_zahtjev->preferirani_rok)) != '01.01.1970') echo date ( "d.m.Y" , strtotime($red_zahtjev->preferirani_rok)); ?>&nbsp;</label2>			
		<div class="clear"></div>
			
		<label for="login">Opis problema : </label>
		<label2 for="login"><? echo $red_zahtjev->opis_zahtjeva; ?>&nbsp;</label2>			
		<div class="clear"></div>
 	...
 	<fieldset>
		...
		<label for="password">Tip zahtjeva : </label>
		    ...
		    <SELECT name="nacin_realizacije" id="nacin_realizacije"  >
			<OPTION SELECTED VALUE=''>
				<?php foreach($nacin_realizacije AS $nacin_realizacije){
				if ($red_zahtjev->nacin_realizacije == $nacin_realizacije[0])
				  echo "<OPTION SELECTED VALUE='".$nacin_realizacije[0]."'> ".$nacin_realizacije[1]."";
				else
				  echo "<OPTION VALUE='".$nacin_realizacije[0]."'> ".$nacin_realizacije[1]."";
				}	
				?>
			</SELECT>			
		    <div class="clear"></div>
			...
			<SELECT name="izvrsioc" id="izvrsioc">
			  <OPTION SELECTED VALUE=''>
				<?php foreach($izvrsioci AS $izvrsioc){			
				if ($red_zahtjev->serviser == $izvrsioc[0])					
			...
			<label for="password">Lokacija : </label>
			<SELECT name="lokacija" id="lokacija">
			  <OPTION SELECTED VALUE=''>
				<?php foreach($lokacije AS $lokacija){
				if ($red_zahtjev->lokacija_izvrsenja == $lokacija[0])
                  echo "<OPTION SELECTED VALUE='".$lokacija[0]."'> ".$lokacija[1]."";	
                else				  
				  echo "<OPTION VALUE='".$lokacija[0]."'> ".$lokacija[1]."";
				}	
				?>
			</SELECT>
			<div class="clear"></div>
			...
			<label for="password">Krajnji rok : </label>
			<input type = "text" name="krajnji_rok" id="krajnji_rok" class="date_input" value="<? if ($red_zahtjev->zadnji_rok != null) echo date ( "d.m.Y" , strtotime($red_zahtjev->zadnji_rok)); ?>">
			<div class="clear"></div>
			
			<label for="password">Broj sati : </label>
			<input type = "text" name="broj_sati" id="broj_sati" value="<? echo $red_zahtjev->broj_sati; ?>">
			<div class="clear"></div>
			
			<label for="password">Opis realizacije : </label>
			<textarea name="opis_realizacije"><? echo $red_zahtjev->opis_realizacije; ?></textarea>
			<div class="clear"></div>
